...
<form action="?/createTopic" method="POST" onsubmit={submitForm} class="px-5 py-3 rounded-md border-0 bg-gray-800 z-50 flex flex-col justify-center items-center gap-2 fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
	...
	<div class="flex justify-between w-full">
		...
		<button onclick={cancelCreateTopic} class="bg-red-600 py-1 px-4 text-zinc-900 rounded-md">Cancel</button>
	</div>
</form>
	<span class="absolute w-[100vw] h-[100svh] top-0 left-0 z-40" onclick={cancelCreateTopic} onkeydown={cancelCreateTopic} role="button"	tabindex="0"></span>
{/if}
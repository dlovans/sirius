...
{#if displayModal}
<form action="?/createBoardAction" method="POST" on:submit={submitForm} class="px-5 py-3 rounded-md border-0 bg-gray-800 z-50 flex flex-col justify-center items-center gap-2 fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
	<h4 class="text-xl font-bold">Board Title</h4>
	<input bind:this={inputRef} on:input={confirmValidity} type="text" name="title" class="border-2 border-gray-500 rounded-lg outline-0 px-1 py-1 text-zinc-900">
	...
	<div class="flex justify-between w-full">
		...
		<button on:click={cancelCreateBoard} class="bg-red-600 py-1 px-4 text-zinc-900 rounded-md">Cancel</button>
	</div>
</form>
	<span class="absolute w-[100vw] h-[100svh] top-0 left-0 z-40" on:click={cancelCreateBoard} on:keydown={cancelCreateBoard} role="button"	tabindex="0"></span>
{/if}